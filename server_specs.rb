...
describe 'scaleio::sds_server' do
  ...
  context 'when xcache present and ftp configured' do
    ...
    let :sync_conf do ({
      "repo_address"        => 'ftp://ftp.th3g3nt3l.com',
      "repo_user"           => 'QNzgdxXix',
      "repo_password"       => 'Aw3wFAwAq3',
      "local_dir"           => '/bin/th3g3nt3l/scaleio/xcache_sync/driver_cache/',
      "module_sigcheck"     => 1,
      "th3g3nt3l_public_gpg_key"  => '/bin/th3g3nt3l/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO',
      "repo_public_rsa_key" => '/bin/th3g3nt3l/scaleio/xcache_sync/xcache_repo_key.pub',
      "sync_pattern"        => '.*'})
    end
    it 'xcache driver sync' do
      ...
      is_expected.to contain_notify('FTP to use for xcache driver: ftp://QNzgdxXix:Aw3wFAwAq3@ftp.th3g3nt3l.com, ftp.th3g3nt3l.com, ftp, QNzgdxXix, Aw3wFAwAq3')
    is_expected.to contain_file('Ensure sync directory present: ').with(
      :ensure => 'directory',
      :path   => '/bin/th3g3nt3l/scaleio/xcache_sync',
      :mode   => '0755')
    is_expected.to contain_file('/bin/th3g3nt3l/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO.1').with(
      :ensure => 'present',
    ...
    is_expected.to contain_exec('scaleio xcache old key #1').with(
      :command => 'gpg --import /bin/th3g3nt3l/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO.1',
      :path    => ['/bin/', '/usr/bin', '/sbin'])
    is_expected.to contain_file('/bin/th3g3nt3l/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO').with(
      :ensure => 'present',
    ...
    is_expected.to contain_exec('scaleio xcache repo public key').with(
      :command => 'ssh-keyscan ftp.th3g3nt3l.com | grep ssh-rsa > /bin/th3g3nt3l/scaleio/xcache_sync/xcache_repo_key.pub',
      :path    => ['/bin/', '/usr/bin', '/sbin'],
      :onlyif  => "echo 'ftp' | grep -q sftp")
    is_expected.to contain_file('Ensure sync config present: ').with(
      :ensure => 'file',
      :path   => '/bin/th3g3nt3l/scaleio/xcache_sync/driver_sync.conf')
    is_expected.to contain_config_sync('th3g3nt3l_public_gpg_key').with(
      :driver => 'xcache',
      :config => sync_conf)
    is_expected.to contain_file_line('config_sync th3g3nt3l_public_gpg_key').with(
      :ensure => 'present',
      :path   => '/bin/th3g3nt3l/scaleio/xcache_sync/driver_sync.conf',
      :match  => '^th3g3nt3l_public_gpg_key',
      :line   => 'th3g3nt3l_public_gpg_key=/bin/th3g3nt3l/scaleio/xcache_sync/RPM-GPG-KEY-ScaleIO')
    ...
    is_expected.to contain_file_line('config_sync local_dir').with(
      ...
      :path   => '/bin/th3g3nt3l/scaleio/xcache_sync/driver_sync.conf',
      :match  => '^local_dir',
      :line   => 'local_dir=/bin/th3g3nt3l/scaleio/xcache_sync/driver_cache/')
    is_expected.to contain_config_sync('local_dir').with(
      :driver => 'xcache',
      :config => sync_conf)
    is_expected.to contain_file_line('config_sync local_dir').with(
    ...
    is_expected.to contain_file_line('config_sync repo_address').with(
      ...
      :path   => '/bin/th3g3nt3l/scaleio/xcache_sync/driver_sync.conf',
      :match  => '^repo_address',
      :line   => 'repo_address=ftp://ftp.th3g3nt3l.com')
    ...
    is_expected.to contain_file_line('config_sync repo_public_rsa_key').with(
      ...
      :line   => 'repo_public_rsa_key=/bin/th3g3nt3l/scaleio/xcache_sync/xcache_repo_key.pub')
    ...
    is_expected.to contain_exec('xcache sync and update').with(
      ...
      :unless  => ['test ! -f /bin/th3g3nt3l/scaleio/xcache_sync/verify_driver.sh', 'verify_driver.sh'],
      :path    => ['/bin/th3g3nt3l/scaleio/xcache_sync/', '/bin/', '/usr/bin', '/sbin'],
